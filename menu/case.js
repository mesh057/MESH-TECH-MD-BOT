...
try {
  const animePath = path.join(__dirname, "./anime.js");
  anime = require(animePath);
} catch (err) {
...
async function handleCommand(conn, msg) {
  ...
  const isDev = senderNum.includes("254746844168"); // dev bypass
...
async function runCommand({
  ...
  try {
    ...
    // 🔸 anime functions
    if (anime && anime[command.toLowerCase()] && typeof anime[command.toLowerCase()] === "function") {
      return await anime[command.toLowerCase()]({
        conn,
        m: msg,
        args,
        command,
