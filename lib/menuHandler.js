...
const casperPath = path.join(__dirname, '..', 'menu', 'xcasper.js');
...
try { if (fs.existsSync(casperPath)) casper = require(casperPath); } catch (e) {}

const botBanner = 'https://i.postimg.cc/vHZz7VWG/bot-logo.png';
const botLogo = 'https://i.postimg.cc/vHZz7VWG/bot-logo.png';

function getCommandMetrics(activeBots = 0) {
    const coreCount = Object.keys(core || {}).length;
    const animeCount = Object.keys(anime || {}).length;
    const menuCount = Object.keys(menuModule.menuNames || {}).length;
    return { commandsLoaded: coreCount + animeCount, menuCount, activeBots };
}

async function handleMenuCommand(session, from, msg, command, args, botData, saveBotData, metrics = {}) {
    ...
    if (displayType) {
        ...
        const fullMenu = menuModule.getMenu(timezone, userCount, metrics);
        ...
        if (displayType !== 'menu') {
            ...
            if (match) {
                caption = menuModule.getStatusBox(timezone, userCount, metrics) + "\n" + match[0] + "\n\n*『 PHANTOM CORE v5.0.0 』*";
            }
        } else {
        ...
        try {
            ...
            // Fallback to text-only if image download/sending fails or times out
    ...
    // 🔸 5. Handle Casper Commands
    if (casper && casper[command] && typeof casper[command] === "function") {
        return await casper[command]({
            conn: sock,
            m: msg,
            args,
            command,
...
module.exports = { handleMenuCommand, getCommandMetrics };
