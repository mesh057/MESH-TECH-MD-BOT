...
function saveCache() {
    try {
        fs.writeFileSync(CACHE_PATH, JSON.stringify([...latexCache.entries()]), "utf-8");
    }
    catch (e) { }
}
            ...
            if (text[i + 1] == "(" || text[i + 1] == "<") {
                ...
                if (type == "latex") {
                    if (!latex)
                        continue;
                    let txt = raw;
                    let width = null, height = null, font_height = 50, padding = 8;
                    ...
                    if (match && match[2] && match[3]) {
                        ...
                        if (match[4])
                            font_height = Number(match[4]);
                        if (match[5])
                            padding = Number(match[5]);
                    }
                    let cleanUrl = url;
                    try {
    ...
    static async resolveLatexDimensions(url, font_height) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (!_s2)
                    return { url, width: 100, height: fh };
            ...
            console.error("[latex] Fetch dimensions failed, using estimation:", e.message);
        }
        let formula = "";
        try {
        ...
        const estHeight = formula.includes("\\frac") ||
            ...
            header: {
                ...
                ...(this._data
                    ? await prepareWAMessageMedia(this._data, {
                        upload: __classPrivateFieldGet(this, _Button_client, "f").waUploadToServer,
                    }).catch((e) => {
...
class ButtonV2 extends BaseBuilder {
    ...
    async build(jid, { mentions, ...options } = {}) {
        let _thumbnail = this._image
            ? await BaseBuilder.resize(Buffer.isBuffer(this._image)
                ? this._image
                : await BaseBuilder.fetchBuffer(this._image, {}, { silent: true }), 300, 300)
            : null;
        const contextInfo = { ...this._contextInfo };
        if (mentions?.length)
            contextInfo.mentionedJid = mentions;
...
class AIRich extends BaseBuilder {
    ...
    addImage(imageUrl, { resolveUrl = false } = {}) {
        ...
        const list = Array.isArray(imageUrl)
            ? imageUrl.map((v) => {
                const url = Toolkit.resolveMedia(__classPrivateFieldGet(this, _AIRich_client, "f"), v, "image", {
                    resolveUrl,
                });
                return {
                    imagePreviewUrl: url,
            ...
            : (() => {
                const url = Toolkit.resolveMedia(__classPrivateFieldGet(this, _AIRich_client, "f"), imageUrl, "image", {
                    resolveUrl,
                });
                return [
                    {
    ...
    async build({ forwarded = true, notification = false, includesUnifiedResponse = true, includesSubmessages = true, quoted, quotedParticipant, ...options } = {}) {
        ...
        const qObj = quoted
            ? {
                ...
                quotedType: 0,
                quotedMessage: typeof quoted === "object" && quoted !== null
        ...
        const sections = this._footer
            ...
            : [...(await waitAllPromises(this._sections))];
        const richResponseSources = await waitAllPromises(this._richResponseSources);
        return {
            messageContextInfo: {
                deviceListMetadata: {},
